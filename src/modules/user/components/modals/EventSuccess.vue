  ...
  <div id="pi-transaction">
    <div class="pi-transaction">
      <div class="info spacer">
        <h3 v-if="type === 'deposit'">Deposit request successful!!!</h3>
        <h3 v-if="type === 'withdrawal'">Withdrawal request successful!!!</h3>
      ...
      <div class="d-flex flex-column align-items-center mt-5" style="gap: 35px">
        ...
        <p class="text-center" v-if="type === 'deposit'">
          Deposit requested, <span class="balance">NGN{{ amount }}</span> will appear in
          your wallet when payment is received and validated.
        </p>
        <p class="text-center" v-if="type === 'withdrawal'">
          Withdrawal requested, <span class="balance">NGN{{ amount }}</span> will be credited to 
          your submitted bank account.
        </p>
        <p class="text-center" v-if="type === 'place_order'">
          You have successfully placed an order of <span class="balance">{{ amount }}Pi</span> the sum of 
          <span class="balance">{{ ngn_amount }}NGN</span> has been deducted from your wallet.
        </p>
      ...
      <div class="text-center mt-4">
        <button class="button primary-btn w-50 py-3" @click="$router.push('/home')">
          Return Home
        </button>
      </div>
    </div>
