<template>
    <div>
      <div class="tw-mb-4">
        <h4 class="tw-text-xl tw-font-semibold tw-mb-0">Cable Subscription</h4>
        <div class="tw-block tw-w-fit" style="width: max-content">
          <span
            class="tw-flex tw-space-x-1 tw-items-center tw-bg-white tw-py-2 tw-px-4 tw-mt-2 tw-rounded-lg"
          >
          ...
          <form @submit.prevent="handleSubmit(buyAirtime)">
            ...
            <div class="tw-mb-3">
              ...
              >
                ...
                >
                  <template #option="{ name, variation_amount }">
                    <h6 class="tw-mb-0 tw-text-sm">{{ name }}</h6>
                    <h6 class="tw-text-light tw-text-sm">
                      {{ `NGN ${variation_amount}` }}
                    </h6>
                  </template>
  ...
  export default {
    ...
    methods: {
      ...
      getVariants() {
        this.isLoading = true;
        this.$request
          .get(`account/bill-payment/service/${this.active}/variants`)
          .then((res) => {
            console.log(res);
            this.variants = res.data.content.variations;
      ...
      buyAirtime() {
        var payload = {
          ...
          variationCode: this.variationCode.variation_code,
        };
        console.log(payload);
          this.loading = true;
          this.$request
            ...
            .then((res) => {
              this.loading = false;
              this.$router.push(`/utilities-success?message=data subscription`);
              this.balances();
              return res;
            })
            .catch((err) => {
    ...
    watch: {
      ...
      variationCode: {
        handler(val) {
          if (val) {
            this.amount = val.variation_amount;
          } else {
            this.amount = 0;
          }
        },
